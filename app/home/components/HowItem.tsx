...
const HowItem: React.FC<Props> = ({ num, title, desc, isDark }) => {
  let number;
  ...
  return (
    <div className="flex max-w-[327px] flex-col gap-6 text-center md:w-full md:max-w-[220px] md:gap-10 md:text-left lg:max-w-[250px] xl:max-w-[285px]">
      ...
      <h3
        className={`text-xxl ${isDark ? "text-LightCream" : "text-DarkGreyBlue"} lg:max-w-[255px] lg:text-[32px]`}
      >
        {title}
      </h3>
      <p
        className={`text-lm ${isDark ? "text-LightCream" : "text-DarkGreyBlue"} lg:text-base`}
      >
        {desc}
      </p>
    </div>
